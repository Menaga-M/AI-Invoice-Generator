...
import RemainderModel from "../../components/invoices/RemainderModel";
...
const AllInvoices =  () => {
  ...
  if(loading) {
    return <div className="flex justify-center items-center h-96"><Loader2 className="justify-center w-8 h-8 text-blue-600 animate-spin"/></div>
  }

  return (
    <div className="space-y-6">
      <CreateWithAIModel isOpen={isAiModelOpen} onClose={() => setIsAiModelOpen(false)}/>
      <RemainderModel isOpen={isRemainderModalOpen} onClose={() => setIsRemainderModalOpen(false)} invoiceId={selectedInvoiceId}/>
                        ...
                        <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium 
                          ...
                          <td className="px-6 py-4 whitespace-nowrap text-right text-sm font-medium">
                            <div className="flex items-center justify-end gap-2" onClick={(e) => e.stopPropagation()}>
                              ...
                              {invoice.status !== 'Paid' && (
                                <Button size="small" variant="ghost" onClick={() => handleOpenRemainderModel(invoice._id)} title="Generate Remainder">
